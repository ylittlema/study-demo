...
public class MMCQ {
    ...
    public static void main(String[] args) throws IOException {
        BufferedImage img = ImageIO.read(new File("C:\\Users\\yjn\\Desktop\\图片主色调\\example.jpg"));
				/* read your image here using Image IO */
        // Then get the 10 most used colors palette (first being the dominant color of the image)
        List<int[]> result = MMCQ.compute(img, 2);
        for (int[] tem:result){
            for (Integer itemInt :tem){
                System.out.print(Integer.toHexString(itemInt));;
            }
            System.out.println();
        }
    }
